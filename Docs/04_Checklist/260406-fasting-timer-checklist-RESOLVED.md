...
|---|---|---|
| H1: StressView sheet switch line precision | HIGH | Fixed insertion point to "after line 173 (`InterventionsView()`), before closing `}` on line 174" with exact code context |
| M1: Force-unwrap `schedule!` | MEDIUM | Replaced with `if let schedule { ... }` safe pattern |
| M2: Missing `scenePhase` observation | MEDIUM | Added `scenePhase` environment + `.onChange` item to Step 2.3 |
| L1: Phase 2 Gate only builds main target | LOW | Acknowledged — acceptable, final post-implementation builds all 4 |
...
- [ ] Create `WellPlate/Features + UI/Stress/Views/FastingView.swift`
  ...
  <!-- RESOLVED: M2 — Added scenePhase environment property -->
...
- [ ] Session lifecycle (in `.onAppear` / `.onChange`):
  <!-- RESOLVED: M1 — Replaced schedule! with if let schedule safe pattern -->
  - [ ] `.onAppear`: configure service safely:
    ```swift
    if let schedule {
        fastingService.configure(schedule: schedule, activeSession: activeSession)
    }
    ```
  <!-- RESOLVED: M2 — Added scenePhase onChange for timer reconstruction on foreground return -->
  - [ ] `.onChange(of: scenePhase)`: reconstruct timer on foreground return:
    ```swift
    .onChange(of: scenePhase) { phase in
        guard phase == .active, let schedule else { return }
        fastingService.configure(schedule: schedule, activeSession: activeSession)
    }
    ```
...
- [ ] Build main target after Phase 2:
  - [ ] `xcodebuild -project WellPlate.xcodeproj -scheme WellPlate -destination 'generic/platform=iOS Simulator' build`
  - Verify: 0 errors
  <!-- RESOLVED: L1 — Acknowledged: Phase 2 gate only builds main target. Final post-implementation builds all 4. Acceptable. -->
...
- [ ] Add `.fasting` case to `StressSheet` enum (~line 12, after `case interventions`):
  ```swift
  case fasting
  ```
  - Verify: Enum compiles

- [ ] Add `case .fasting: return "fasting"` to the `id` computed property (~line 21–31, after the `.interventions` return)
  - Verify: All `id` switch cases exhaustive

- [ ] Add "Fast" button to toolbar `Menu` (after the "Resets" button at ~line 88):
  ```swift
  Button {
...
<!-- RESOLVED: H1 — Precise insertion point with exact surrounding code context -->
- [ ] Add `case .fasting:` to `.sheet(item: $activeSheet)` switch. Insert between line 173 (`InterventionsView()`) and the closing `}` on line 174. Surrounding context:
  ```swift
  case .interventions:           // line 172 (existing)
      InterventionsView()        // line 173 (existing)
  case .fasting:                 // ← NEW (insert here)
      FastingView()              // ← NEW
  }                              // line 174 → becomes line 176 (existing closing brace)
  ```
...
- [ ] Smoke test in Simulator:
  ...
  - [ ] Background and foreground app → verify timer reconstructs correctly

# Implementation Checklist: Home Screen Content Customisation
...
- [ ] Add stored property after `var sleepGoalHours: Double` (~line 37):
  ...
  ```
- [ ] Add `homeLayoutJSON: String = "{}"` as the last parameter of the `init(...)` method
- [ ] Add `self.homeLayoutJSON = homeLayoutJSON` in the init body
...
- [ ] Call `migrateHideInsightCardIfNeeded()` in the `.onAppear` block (after existing calls)

# Implementation Plan: AI Insights V2 (RESOLVED)

## Audit Resolution Summary

| ID | Severity | Issue | Resolution |
|---|---|---|---|
| H1 | HIGH | `InsightCard` needs `Equatable`/`Hashable` for `.sheet(item:)` | Added manual ID-only conformance to `InsightCard` in Step 1.1 |
| H2 | HIGH | `ContextualBarState.stressActionable` "See Insight" UX change | Added explicit note in Step 5.1 confirming intentional UX change |
| M1 | MEDIUM | Font convention — plan uses `.system(size:)` instead of `.r()` | Added note to all Phase 4 steps: use `.r()` extension where semantic sizes apply |
| M2 | MEDIUM | `StressScoring` dependency not mentioned | Added note in Step 2.1 confirming `InsightEngine` reads pre-computed scores |
| M3 | MEDIUM | No auto-generation on `onAppear` | Added `Task { await insightEngine.generateInsights() }` to Step 5.1 `onAppear` |
| M4 | MEDIUM | `Color` in `InsightChartData.comparisonBars` isn't `Sendable` | Replaced `Color` with `WellnessDomain` in chart data; view resolves colour |
| M5 | MEDIUM | `ProgressInsightsView` overlap not addressed | Added to Non-Goals section |
| M6 | MEDIUM | `MacroRadarChart` listed as primary but is complex | Marked as optional/stretch; `MacroGroupedBarChart` is the primary implementation |
| L1 | LOW | No haptic feedback on insight card interactions | Acknowledged — add haptics as polish pass |
| L2 | LOW | No accessibility labels on chart components | Acknowledged — add `.accessibilityLabel` as post-MVP task |
| L3 | LOW | `WellnessDaySummary` has 25+ flat fields | Acknowledged — matches existing `StressInsightDaySummary` pattern; acceptable |
...
- Charts per insight type: area trend, scatter plot, comparison bars, grouped bar (macro), milestone ring, sparkline
...
| `WellPlate/Features + UI/Home/Components/InsightCharts.swift` | **Create** | Reusable chart subviews: `TrendAreaChart`, `CorrelationScatterChart`, `ComparisonBarChart`, `MacroGroupedBarChart`, `MilestoneRingView`, `SparklineView` |
...
// <!-- RESOLVED: M4 — Replaced Color with WellnessDomain in comparisonBars. View layer resolves to Color via domain.accentColor. -->
...
// <!-- RESOLVED: H1 — Added manual Equatable/Hashable using id-only comparison for .sheet(item:) -->
...
<!-- RESOLVED: M2 — InsightEngine reads pre-computed StressReading.score values from SwiftData. It does NOT need StressScoring.swift for raw-vital scoring. If future detectors need to re-score from vitals, import StressScoring at that point. -->
...
3. Concurrent HealthKit fetches using `async let`:
   ...
   - `fetchExerciseMinutesSafely` → `[DailyMetricSample]` (uses `healthService.fetchExerciseMinutes`)
...
For each macro (protein, fiber, fat, carbs):
  ...
  - chartData: .macroRadar(actual: {...}, goals: {...})
...
<!-- RESOLVED: M1 — Use `.r()` font extension for semantic sizes (headline, caption, footnote, etc.). Fall back to `.system(size:)` only for non-standard sizes. -->
...
struct TrendAreaChart: View {
    ...
    // Font: .r(.caption2, .medium) for axis labels
    // Frame height: 130
}
```
...
<!-- RESOLVED: M4 — bars use WellnessDomain, view resolves to domain.accentColor -->
```swift
struct ComparisonBarChart: View {
    let bars: [(label: String, value: Double, domain: WellnessDomain)]
    let highlight: Int?
    // Body: BarMark (horizontal) per bar, coloured by domain.accentColor
    // Frame height: 80
}
```

**`MacroGroupedBarChart`** — Grouped vertical bars for macro actual vs. goal:
<!-- RESOLVED: M6 — MacroGroupedBarChart is the primary implementation for .macroRadar data. Canvas-based radar chart deferred to post-MVP polish. -->
```swift
struct MacroGroupedBarChart: View {
    let actual: [String: Double]  // protein, carbs, fat, fiber
    let goals: [String: Double]   // same keys
    // Body: grouped BarMark — actual bar + goal bar per macro
    // Uses AppColors.brand for actual, .secondary.opacity(0.3) for goal
    // Frame height: 130
}
```
...
- Axis labels: `.r(.caption2, .regular)` where possible, `.system(size: 9, design: .rounded)` for chart-specific sizing
- Interpolation: `.catmullRom` for line charts (matching existing pattern)

<!-- RESOLVED: L2 — Acknowledged: Add .accessibilityLabel to each chart subview as a post-MVP polish pass. Each label should summarise the chart data (e.g., "Stress trend: declining from 72 to 48 over 7 days"). -->

---

### Phase 4: View Layer

<!-- RESOLVED: M1 — All new view code in Phase 4 uses .r() font extension for semantic sizes. -->
...
<!-- RESOLVED: L1 — Acknowledged: Add HapticService.impact(.light) on DailyInsightCard appear when a new insight is surfaced. Add HapticService.impact(.medium) on regenerate. Implement as polish pass during or after Phase 4. -->
...
struct InsightsHubView: View {
    ...
    // InsightCard conforms to Identifiable + Equatable + Hashable (H1 resolved)
...
Note: Move the `InsightEntrance` modifier from `HomeAIInsightView.swift` (lines 602-623) into a shared location (bottom of `InsightCardView.swift` or a new `InsightAnimations.swift`) since the old view will be removed.
...
6. **Update onAppear** (line 239):
   <!-- RESOLVED: M3 — Added auto-generation on onAppear. Same-day cache prevents redundant regeneration. -->
   ```swift
...
7. **Update contextual bar `onSeeInsight`** (lines 193-195):
   <!-- RESOLVED: H2 — The "See Insight" action from all contextual bar states (including .stressActionable) now opens InsightsHubView, which is a multi-domain hub. This is the intended UX change: the hub includes stress insights alongside nutrition, sleep, activity, etc. The bar's onStressTab still navigates to the dedicated Stress tab for stress-specific actions. -->
   ```swift
   onSeeInsight: {
       showInsightsHub = true
       Task { await insightEngine.generateInsights() }
...
#### Step 5.2: Verify `ContextualActionBar.swift` (no changes needed)
...
**Action**: Move the `InsightEntrance` ViewModifier + `.insightEntrance(index:)` extension from `HomeAIInsightView.swift` into `InsightCardView.swift` so both `InsightsHubView` and `InsightDetailSheet` can use it.
**Dependencies**: Step 4.1
**Risk**: Low
...
- **Canvas-based radar chart** — Deferred to post-MVP polish. `MacroGroupedBarChart` serves the `.macroRadar` data type initially.
<!-- RESOLVED: M5 — ProgressInsightsView is NOT deprecated. It serves a different navigation path (accessed from Food Journal / Progress). The new InsightsHubView provides cross-domain insights from the Home screen. Long-term consolidation is a future task, not in scope. -->
- **ProgressInsightsView deprecation** — `ProgressInsightsView` remains as-is. It serves the Food Journal / Progress navigation path. Long-term consolidation with InsightsHubView is a future task.
...
7. **Auto-generation**: Cold launch → verify DailyInsightCard populates automatically without user action
8. **Dark mode**: Toggle dark mode → verify all charts and cards render correctly
9. **VoiceOver**: Navigate InsightsHubView with VoiceOver → verify all cards are accessible
...
- [ ] DailyInsightCard auto-populates on app launch (no manual trigger needed)
...
- [ ] At least 4 chart types render correctly (trend, scatter, comparison bars, sparkline)
- [ ] `.macroRadar` data renders as grouped bar chart (not radar)
...
- [ ] `.r()` font extension used for semantic sizes in new views

...
struct WellnessReportGenerator {
    ...
    static func generateCSV(
        ...
        var rows: [String] = ["date,stress_score,calories,protein_g,carbs_g,fat_g,fiber_g,steps,water_glasses,mood"]
        ...
        for day in daySeq {
            ...
            let row = "\(dateFmt.string(from: day)),\(stressStr),\(calories),\(String(format: "%.1f", protein)),\(String(format: "%.1f", carbs)),\(String(format: "%.1f", fat)),\(String(format: "%.1f", fiber)),\(steps),\(water),\(mood)"
            rows.append(row)
        }
